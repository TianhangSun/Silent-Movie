...
BEGIN_MESSAGE_MAP(CViewEdit, CScrollView)
    ...
    ON_WM_LBUTTONDOWN()
    ON_WM_MOUSEMOVE()
	ON_COMMAND(ID_SEED_TREE, &CViewEdit::OnSeedTree)
	ON_COMMAND(ID_SEED_TREE32787, &CViewEdit::OnSeedTree32787)
	ON_COMMAND(ID_EDIT_HARVESTFRAME, &CViewEdit::OnEditHarvestframe)
END_MESSAGE_MAP()
...
/**set seed for tree0*/
void CViewEdit::OnSeedTree()
{
	for (auto actor : *GetPicture())
	{
		if (actor->GetName() == L"trees0")
		{
			actor->SetSeed();
			UpdateObserver();
		}
...
/**set seed for tree*/
void CViewEdit::OnSeedTree32787()
{
	for (auto actor : *GetPicture())
	{
		if (actor->GetName() == L"trees")
		{
			actor->SetSeed();
			UpdateObserver();
		}
...
/**edit harvest frame*/
void CViewEdit::OnEditHarvestframe()
{
	CHarvestDlg dlg;
	for(auto actor : *GetPicture())
	{
		if (actor->GetName() == L"basket")
		{
			dlg.SetHarvestFrame(actor->GetHarvestFrame() + 100);
			UpdateObserver();
		}
	}
	if (dlg.DoModal() == IDOK)
	{
		for (auto actor : *GetPicture())
		{
			if (actor->GetName() == L"basket")
			{
				actor->SetHarvestFrame(dlg.GetHarvestFrame());
				UpdateObserver();
			}
		}
	}
