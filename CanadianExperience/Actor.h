...
public:
    ...
    * \returns Pointer to animation channel */
	...
	/**save the seed
	* \returns the seed*/
	int SaveSeed(){ for (auto i : mDrawablesInOrder){ return i->SaveSeed(); } }

	/**load the seed
	* \param seed to set*/
	void LoadSeed(int seed){ for (auto i : mDrawablesInOrder){ i->LoadSeed(seed); } }

	/**set harvest frame
	* \param frame the frame to set*/
	void SetHarvestFrame(int frame){ for (auto i : mDrawablesInOrder){ i->SetHarvestFrame(frame); } }

	/**get harvest frame
	* \returns harvest frame*/
	int GetHarvestFrame(){ for (auto i : mDrawablesInOrder){ return i->GetHarvestFrame(); } }
