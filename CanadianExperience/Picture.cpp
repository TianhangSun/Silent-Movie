...
void CPicture::Save(const std::wstring &filename)
{
	...
	for (auto actor : mActors)
	{
		...
		{
			root->SetAttribute(L"trees", actor->SaveSeed());
		}

		else if (actor->GetName() == L"trees0")
		{
			root->SetAttribute(L"trees0", actor->SaveSeed());
		}

		else if(actor->GetName() == L"basket")
		{
			root->SetAttribute(L"basket", actor->GetHarvestFrame()+100);
		}
	}
...
void CPicture::Load(const std::wstring &filename)
{
    ...
    try
    {
		...
		for (auto actor : mActors)
		{
			if (actor->GetName() == L"trees")
			{
				actor->LoadSeed(root->GetAttributeIntValue(L"trees", 9187));
			}

			else if (actor->GetName() == L"trees0")
			{
				actor->LoadSeed(root->GetAttributeIntValue(L"trees0", 2));
			}

			else if (actor->GetName() == L"basket")
			{
				actor->SetHarvestFrame(root->GetAttributeIntValue(L"basket", 230));
			}
		}
