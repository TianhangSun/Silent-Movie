...
void CViewTimeline::OnDraw(CDC* pDC)
{
	...
	{
		mFirstDraw = false;
		SetTimer(1, timeline->GetFrameRate(), nullptr);
 ...
 */
 ...
 /**function that update animation*/
 void CViewTimeline::AutoPlay()
 {
	 /*CTimeline *timeline = GetPicture()->GetTimeline();
	 if (timeline->IsPlaying() == true && mDuration > (1 / (double)timeline->GetFrameRate()))
	 {
